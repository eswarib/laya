...
async function main() {
  // Backward compatible shim: prefer LAYA_* vars, but support SMARTOS_* for older setups.
  const cwd = process.env.LAYA_ROOT
    ? path.resolve(process.env.LAYA_ROOT)
    : process.env.SMARTOS_ROOT
      ? path.resolve(process.env.SMARTOS_ROOT)
      : path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');

  const host = process.env.OLLAMA_HOST ?? 'http://127.0.0.1:11434';
  const model = process.env.OLLAMA_MODEL ?? 'llama3.2:3b';
  const startupTimeoutMs =
    Number(process.env.LAYA_OLLAMA_STARTUP_TIMEOUT_MS ?? process.env.SMARTOS_OLLAMA_STARTUP_TIMEOUT_MS ?? '15000') ||
    15000;
  ...
  if (!(await isOllamaUp(host))) {
    ...
    if (!ok) {
      process.stderr.write(
        `smartos: Ollama did not become ready at ${host} within ${startupTimeoutMs}ms.\n` +
  ...
  child.on('exit', (code, signal) => {
    ...
    if (startedPid && (process.env.LAYA_KILL_OLLAMA_ON_EXIT === '1' || process.env.SMARTOS_KILL_OLLAMA_ON_EXIT === '1')) {
      try {
...
main().catch(err => {
  process.stderr.write(`smartos launcher error: ${err instanceof Error ? err.message : String(err)}\n`);
  process.exit(1);
});
